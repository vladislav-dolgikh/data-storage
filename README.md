...
Этап определения пиков ***ChIP-seq*** необходим для локализации районов связывания ТФ в геноме. В рамках данного конвейера определение пиков осуществляется при помощи инструмента **MACS3**. Существует несколько подходов к определению пиков, схематично изображенном на схеме:
...
|Data directory name|Название файла, депонированного в хранилище. Имеет вид `Название изучаемого ТФ` + `Время обработки образца` + `Тип режима определения пиков` + `Первый автор статьи, в которой опубликован эксперимент`. Например, `ARR1_4h_bdgdiff_Xie.bed`|
